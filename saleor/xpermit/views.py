...
""" 
By default, if anybody goes to the xpermit page, index gets called.

But since we have technicians operating out of the same path (xpermit).. 
...
def index(request):
    ...
    return TemplateResponse(request, 'xpermit/index.html', {'permit': permit} )
"""
...
def technicians_list(request):
	...
	ctx = {'technician': tech}
	return TemplateResponse(request, 'xpermit/technician_list.html', ctx)
...
def new_technician(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			tech = Technician.objects.prefetch_related('lines');
			return TemplateResponse(request, 'xpermit/technician_list.html', {'technician': tech})
	else:
		form = TechnicianForm()
		
	return TemplateResponse(request, 'xpermit/new_technician.html', {'form': form })
...
def permit_form(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            obj.user_id = current_user.id
            form.save()
            return TemplateResponse(request, 'xpermit/index.html')
    # if a GET (or any other method) we'll create a blank form
    else:
        form = PermitForm()
	
    return TemplateResponse(request, 'xpermit/permit_form.html', {'form': form })    
    

class TechnicianList(ListView):
	model = Technician
	template_name = 'xpermit/technician_list.html'
...
class PermitList(ListView):
	#model = Permit
	template_name = 'xpermit/index.html'
